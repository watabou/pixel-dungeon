...
public class Freezing extends Buff implements Hero.Doom {

	private static final String TXT_FROZEN_UP		= "%s cold damage!";
	private static final String TXT_FROZEN_TO_DEATH	= "You frozen to death...";
	...
	public boolean act() {
		...
		if (target.isAlive()) {
			...
			if (target instanceof Hero) {
				...
				if (item instanceof Scroll) {
					...
					GLog.w( TXT_FROZEN_UP, item.toString() );
		...
		if (Level.flamable[target.pos]) {
			GameScene.add( Blob.seed( target.pos, 4, Fire.class ) ); // ice.class 제작
		}
	...
	public void onDeath() {
		...
		Badges.validateDeathFromFREEZING();
		
		Dungeon.fail( Utils.format( ResultDescriptions.FREEZING, Dungeon.depth ) );
		GLog.n( TXT_FROZEN_TO_DEATH );
	}
}
